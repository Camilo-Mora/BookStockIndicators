...
## 137 Volume Profile {-}
Displays  the price at which volume is being traded. The price with the most volume is call point control (POC). It can help to reveal support prices. Deviations from POC can reveal  short term deviations. From the volume profile one can also calculate the POCPRice or the price at which most ticks occurred (this does not indicate how much volume is being traded but how many people is trading, can be calcualted only when such data are available). The volume profile can also indicate the value area, which is the low and high price of the volume profile.
