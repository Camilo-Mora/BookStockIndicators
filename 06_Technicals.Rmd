...
Technical indicators are mathematical calculations based on historic price and/or, volume that aims to forecast stock direction.

Generally speaking they can be grouped in four different categories:
...
These indicators describe variations over the number of stocks traded (commonly, volume is shown as the bars at the bottom of most stock prices).  It is expected that any strong trend should be based on large volume traded. Volume is like the foundation over which price should be built.
...
## Purpose of tecnical indicators {-}
...
Technical indicators can give you hints into the stock's next move: Position, direction, intention, volatility or a trigger.
...
### Triggers {--}
Technical indicators at times cross critical points that can be used as specific entry points. 



```{r, out.width = "100%", echo= FALSE, fig.align = 'center',fig.cap = 'Price cross over bollinger bands as entry trigger'}
knitr::include_graphics("images/CrossOver.png")
```
